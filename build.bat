...
@REM xcopy "%~dp0\ui\" "%~dp0\tmp_ui\" /i /e /y
...
@REM xcopy "%~dp0\tmp_ui" "%~dp0\dist\ui" /i /e /y /c /q
...
@REM powershell.exe -nologo -noprofile -command "& { Compress-Archive -Force -Path '%~dp0\dist\ui', '%~dp0\dist\script_hooks', '%~dp0\dist\scripts' -DestinationPath '%~dp0\dist\%modname%_%version%.zip' }"
